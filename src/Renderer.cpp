...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    VkDescriptorSetLayoutBinding allBladesLayoutBinding = {};
    allBladesLayoutBinding.binding = 0;
    allBladesLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    allBladesLayoutBinding.descriptorCount = 1;
    allBladesLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT; 
    allBladesLayoutBinding.pImmutableSamplers = nullptr;

    VkDescriptorSetLayoutBinding outputBladesLayoutBinding = {};
    outputBladesLayoutBinding.binding = 1;
    outputBladesLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    outputBladesLayoutBinding.descriptorCount = 1;
    outputBladesLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    outputBladesLayoutBinding.pImmutableSamplers = nullptr;

	// output number of grass blades after culling
    ...
    std::array<VkDescriptorSetLayoutBinding, 3> bindings = { allBladesLayoutBinding, outputBladesLayoutBinding, numBladesLayoutBinding };
    VkDescriptorSetLayoutCreateInfo layoutInfo{};
    layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    layoutInfo.bindingCount = static_cast<uint32_t>(bindings.size());
    layoutInfo.pBindings = bindings.data();
...
void Renderer::CreateDescriptorPool() {
    ...
    std::vector<VkDescriptorPoolSize> poolSizes = {
        ...
        // Input blades buffer, output blades buffer, and num blades buffer.
...
void Renderer::CreateComputeDescriptorSets() {
	...
	// by default, scene has 1 Blades object (inside which there are NUM_BLADES blades)
    ...
    for (int i = 0; i < scene->GetBlades().size(); ++i) {
		...
		inputBladeBufferInfo.buffer = blade->GetBladesBuffer();
        inputBladeBufferInfo.offset = 0;
        inputBladeBufferInfo.range = sizeof(Blade) * NUM_BLADES;
        descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        ...
        descriptorWrites[0].descriptorCount = 1;
        descriptorWrites[0].pBufferInfo = &inputBladeBufferInfo;

        VkDescriptorBufferInfo outputBladesBufferInfo = {};
        outputBladesBufferInfo.buffer = blade->GetCulledBladesBuffer();
        ...
        numBladesBufferInfo.buffer = blade->GetNumBladesBuffer();
        ...
        descriptorWrites[2].pBufferInfo = &numBladesBufferInfo;
...
// swapchain imageview and depth imageview
...
void Renderer::RecordComputeCommandBuffer() {
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		// 0, 1, 2: starting descriptor set index, match the pipeline descriptorSetLayouts order
...
void Renderer::RecordCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
		...
		// synchronization between compute shader and indirect draw on graphics pipeline
        ...
        for (uint32_t j = 0; j < barriers.size(); ++j) {
            ...
            /*
            Each buffer/image has an owning queue family at any given time.
            If you write to a buffer on a compute queue and then want to read from it on a graphics queue, you need to:
            Release ownership from the compute family (source).
            Acquire ownership on the graphics family (destination).
            */
            ...
            barriers[j].buffer = scene->GetBlades()[j]->GetNumBladesBuffer();       // the buffer to protect
            barriers[j].offset = 0;
        ...
        for (uint32_t j = 0; j < scene->GetBlades().size(); ++j) {
			...
			// still binding the modelDescriptorSet for now, cuz model matrix is identical matrix
            vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelineLayout, 1, 1, &modelDescriptorSets[j], 0, nullptr);
